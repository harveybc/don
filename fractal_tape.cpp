...
int FractalTape::push_instruction(FractalCmd instr){  ///< Adiciona una instrucción al final de la cinta
    full_tape.push_back(instr);
}
int FractalTape::pop_instruction(FractalCmd &output){  ///< Adiciona una instrucción al final de la cinta
    output=full_tape.front();
}

int FractalTape::get_tape(std::deque <FractalCmd> &output_tape){ ///< Obtiene la cinta de instrucciones
